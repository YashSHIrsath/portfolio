<x-admin-layout>
    <x-slot name="title">Add Tech Stack</x-slot>
    ...
    <div class="max-w-2xl mx-auto py-8">
        ...
        <div class="bg-white dark:bg-[#161b22] shadow rounded-lg border border-slate-200 dark:border-slate-800 p-6">
            <h1 class="text-2xl font-bold mb-6 text-slate-800 dark:text-slate-100">Add New Tech</h1>
            ...
            <form action="{{ route('admin.tech-stacks.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="w-full px-3 py-2 border rounded-md dark:bg-[#0d1117] dark:border-slate-700 dark:text-slate-100 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g. Laravel" required>
                ...
                <div class="mb-4">
                    ...
                    <select name="icon_class" id="icon-select" class="w-full px-3 py-2 border rounded-md dark:bg-[#0d1117] dark:border-slate-700 dark:text-slate-100 focus:ring-blue-500 focus:border-blue-500 font-mono" size="5">
                        ...
                        @foreach($icons as $class => $label)
                            <option value="{{ $class }}" {{ old('icon_class') == $class ? 'selected' : '' }} data-label="{{ strtolower($label) }} {{ strtolower($class) }}">
                                {{ $label }} ({{ $class }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="url" name="url" value="{{ old('url') }}" class="w-full px-3 py-2 border rounded-md dark:bg-[#0d1117] dark:border-slate-700 dark:text-slate-100 focus:ring-blue-500 focus:border-blue-500" placeholder="https://...">

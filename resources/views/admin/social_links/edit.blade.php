<x-admin-layout>
    <x-slot name="title">Edit Social Link</x-slot>

    <div class="max-w-2xl mx-auto py-8">
        <div class="mb-6 flex justify-between items-center">
            <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100">Edit Social Link</h1>
        ...
        <div class="bg-white dark:bg-[#161b22] shadow rounded-lg p-6 border border-slate-200 dark:border-slate-800">
            <form action="{{ route('admin.social-links.update', $socialLink) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="platform" value="{{ old('platform', $socialLink->platform) }}" class="w-full px-3 py-2 border rounded-md dark:bg-[#0d1117] dark:border-slate-700 dark:text-slate-100 focus:ring-blue-500 focus:border-blue-500" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="url" name="url" value="{{ old('url', $socialLink->url) }}" class="w-full px-3 py-2 border rounded-md dark:bg-[#0d1117] dark:border-slate-700 dark:text-slate-100 focus:ring-blue-500 focus:border-blue-500" required>
                ...
                <div class="mb-4">
                    ...
                    <select name="icon_class" class="w-full px-3 py-2 border rounded-md dark:bg-[#0d1117] dark:border-slate-700 dark:text-slate-100 focus:ring-blue-500 focus:border-blue-500 font-mono">
                        ...
                        @foreach($icons as $class => $label)
                            <option value="{{ $class }}" {{ old('icon_class', $socialLink->icon_class) == $class ? 'selected' : '' }}>
                                {{ $label }} ({{ $class }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $socialLink->sort_order) }}" class="w-full px-3 py-2 border rounded-md dark:bg-[#0d1117] dark:border-slate-700 dark:text-slate-100 focus:ring-blue-500 focus:border-blue-500">
                    @error('sort_order') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="flex justify-end gap-4">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded shadow-sm transition-colors">
                        Update Link
                    </button>
                </div>
            </form>
